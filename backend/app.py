...
def register():
    ...
    user = mongo.db.users.find_one({"phone_number": formatted_number})
    if user:
        return jsonify({"success": False, "message": "User already exists"}), 400

    mongo.db.users.insert_one({"phone_number": formatted_number, "points": 0})
    access_token = create_access_token(identity=formatted_number)
    return jsonify({"success": True, "token": access_token, "phone_number": formatted_number})
...
def check_number():
    ...
    entry = mongo.db.spam_numbers.find_one({"Caller ID Number": formatted_number})
    ...
    if entry:
        return jsonify({
            ...
            "blockCount": entry.get("block_count", 0)
        })
    else:
        return jsonify({
            "success": True,
...
def report_number():
    ...
    formatted_number = format_phone_number(number)
    record = mongo.db.spam_numbers.find_one({"Caller ID Number": formatted_number})

    if record:
        report_count = record.get('report_count', 0) + 1
        block_count = record.get('block_count', 0)
        total_reports = report_count + block_count
        is_spam = total_reports >= 4

        mongo.db.spam_numbers.update_one(
            {"Caller ID Number": formatted_number},
            {"$set": {
    ...
    return jsonify({"success": True, "message": "Number reported successfully.", "points": new_points})
...
def block_number():
    ...
    if record:
        block_count = record.get('block_count', 0) + 1
    ...
    # Update user points
    user = mongo.db.users.find_one({"phone_number": current_user})
    new_points = user.get("points", 0) + 20
    mongo.db.users.update_one(
        {"phone_number": current_user},
        {"$set": {"points": new_points}}
    )
